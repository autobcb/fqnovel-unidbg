...
public class FullBookDownloadService {
     ...
     * 全本下载（流式返回）
     */
    public Flux<FullBookDownloadResponse> downloadFullBook(FullBookDownloadRequest request,DeviceManagementService deviceManagementService) {
            ...
            CompletableFuture.runAsync(() -> {
                try {
                    ...
                    if (bookInfo == null) {
                        ...
                        FQNovelResponse<FQNovelBookInfo> bookResponse = fqNovelService.getBookInfo(request.getBookId(),0,deviceManagementService).get();
                    ...
                    for (int batchIndex = 0; batchIndex < totalBatches; batchIndex++) {
                        try {
                            ...
                            FQNovelResponse<FQBatchChapterResponse> batchResponse = 
                                fqNovelService.getBatchChapterContent(batchRequest,deviceManagementService).get();
     ...
     * 获取下载进度
     */
    public CompletableFuture<Map<String, Object>> getDownloadProgress(String bookId,DeviceManagementService deviceManagementService) {
        return CompletableFuture.supplyAsync(() -> {
            try {
                // 优先从Redis获取书籍信息
                ...
                if (bookInfo == null) {
                    // 从API获取
                    FQNovelResponse<FQNovelBookInfo> bookResponse = fqNovelService.getBookInfo(bookId,0,deviceManagementService).get();
                    if (bookResponse.getCode() != 0 || bookResponse.getData() == null) {
                ...
                if (totalChapters <= 0) {
                    try {
                        FQNovelResponse<FQNovelBookInfo> fresh = fqNovelService.getBookInfo(bookId,0,deviceManagementService).get();
     ...
     * 自动检查并恢复下载
     */
    public CompletableFuture<AutoResumeResult> autoResumeDownload(String bookId,DeviceManagementService deviceManagementService) {
        return CompletableFuture.supplyAsync(() -> {
            try {
                log.info("开始自动检查下载进度 - bookId: {}", bookId);
                
                // 获取当前下载进度
                Map<String, Object> progress = getDownloadProgress(bookId,deviceManagementService).get();
                ...
                // 异步执行下载，不等待完成
                downloadFullBook(request,deviceManagementService)
                    .doOnNext(response -> {
     ...
     * 检查所有未完成的任务并自动恢复下载
     */
    public CompletableFuture<AutoResumeAllResult> autoResumeAllDownloads(DeviceManagementService deviceManagementService) {
        return CompletableFuture.supplyAsync(() -> {
            try {
                ...
                for (String bookId : incompleteBooks) {
                    try {
                        AutoResumeResult result = autoResumeDownload(bookId,deviceManagementService).get();
                        if (result.isSuccess()) {
     ...
     */
    public CompletableFuture<CheckAllStatusResult> checkAllDownloadStatus(DeviceManagementService deviceManagementService) {
        return CompletableFuture.supplyAsync(() -> {
            try {
