...
public class FQSearchService {
     ...
     * @return 书籍目录
     */
    public CompletableFuture<FQNovelResponse<FQDirectoryResponse>> getBookDirectory(FQDirectoryRequest directoryRequest,int i,DeviceManagementService deviceManagementService) {
        return CompletableFuture.supplyAsync(() -> {
            try {
                FqVariable var = getDefaultFqVariable();
                ...
                if(i ==0){
                    try {
                        ...
                        return getBookDirectory(directoryRequest,1,deviceManagementService).get();
                    } catch (Exception e1) {
