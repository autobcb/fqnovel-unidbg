...
*/
     ...
     * @return 书籍信息
     */
    public CompletableFuture<FQNovelResponse<FQNovelBookInfo>> getBookInfo(String bookId,int i,DeviceManagementService deviceManagementService) {
        return CompletableFuture.supplyAsync(() -> {
            try {
                // 验证bookId参数
                ...
                // 调用目录接口获取书籍信息
                FQNovelResponse<FQDirectoryResponse> directoryResponse = fqSearchService.getBookDirectory(directoryRequest,0,deviceManagementService).get();
                ...
                if(i ==0){
                    try {
                        var d=new DeviceRegisterRequest();
                        d.setDeviceType("WLZ-AN00");
                        d.setDeviceBrand("HUAWEI");
                        deviceManagementService.registerDevice(d);
                        return getBookInfo(bookId,1,deviceManagementService).get();
                    } catch (Exception e1) {
                        return FQNovelResponse.error("获取章节内容失败: " + e1.getMessage());
                    }
                }
     ...
     * @return 章节内容
     */
    public CompletableFuture<FQNovelResponse<FQNovelChapterInfo>> getChapterContent(FQNovelRequest request,int i,DeviceManagementService deviceManagementService) {
        return CompletableFuture.supplyAsync(() -> {
            try {
                ...
                try {
                    ...
                    throw new Exception("解密章节内容失败: " + e.getMessage());
                ...
                if(i ==0){
                    log.warn("尝试重新注册");
                    try {
                        deviceManagementService.restartProject();
                        return getChapterContent(request,1,deviceManagementService).get();
                    } catch (Exception e1) {
                        return FQNovelResponse.error("获取章节内容失败: " + e1.getMessage());
                    }
                }
     ...
     * @return 批量章节响应
     */
    public CompletableFuture<FQNovelResponse<FQBatchChapterResponse>> getBatchChapterContent(FQBatchChapterRequest request,DeviceManagementService deviceManagementService) {
        return CompletableFuture.supplyAsync(() -> {
            try {
                // 验证参数
                ...
                if (request.getChapterIds() != null && !request.getChapterIds().isEmpty()) {
                    ...
                    if (isChapterPositions(chapterIds)) {
                        ...
                        itemIds = getItemIdsByChapterPositions(request.getBookId(), chapterIds,deviceManagementService);
                    }
                }
     ...
     * @return 对应的itemIds列表
     */
    private List<String> getItemIdsByChapterPositions(String bookId, List<String> chapterPositions,DeviceManagementService deviceManagementService) {
        List<String> itemIds = new ArrayList<>();
